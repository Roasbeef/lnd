...
type ClosingSigs struct {
	// CloseNoClosee is a signature that excludes thte output of the
	// clsoee.
...
func decodeClosingSigs(c *ClosingSigs, tlvRecords ExtraOpaqueData) error {
	...
	// TODO(roasbeef): after rebase, make pbulic method on tlvRecords, get
	// rid of GetTlvType usage

	// TODO(roasbeef): helper func to made decode of the optional vals
	// easier?

	if val, ok := typeMap[tlv.GetTypeVal(tlv.TlvType1{})]; ok && val == nil {
		c.CloserNoClosee = tlv.SomeRecordT(sig1)
	}
	if val, ok := typeMap[tlv.GetTypeVal(tlv.TlvType2{})]; ok && val == nil {
		c.NoCloserClosee = tlv.SomeRecordT(sig2)
	}
	if val, ok := typeMap[tlv.GetTypeVal(tlv.TlvType3{})]; ok && val == nil {
