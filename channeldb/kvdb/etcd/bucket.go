...
// isValidBucketID checks if the passed slice is the required length
// to be a valid bucket id.
func isValidBucketID(s []byte) bool {
	return len(s) == bucketIDLength
}

// makeKey concatenates prefx, parent and key into one byte slice.
// The prefix indicates the use of this key (whether bucket, value or
// sequence), while parentID refers to the parent bucket.
...
// makeBucketKey returns a bucket key from the passed parent
// bucket id and the key.
func makeBucketKey(parent []byte, key []byte) []byte {
	return makeKey(bucketPrefix, parent, key)
}

// makeValueKey returns a value key from the passed parent
// bucket id and the key.
func makeValueKey(parent []byte, key []byte) []byte {
	return makeKey(valuePrefix, parent, key)
}

// makeSequenceKey returns a sequence key of the
// passed parent bucket id.
func makeSequenceKey(parent []byte) []byte {
	return makeKey(sequencePrefix, parent, nil)
}

// makeBucketPrefix returns the bucket prefix of
// the passed parent bucket id. This prefix is
// used for all sub buckets.
func makeBucketPrefix(parent []byte) []byte {
	return makePrefix(bucketPrefix, parent)
}

// makeValuePrefix returns the value prefix of
// the passed parent bucket id. This prefix is used
// for all key/values in the bucket.
