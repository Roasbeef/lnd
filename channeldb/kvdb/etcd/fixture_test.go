...
const (
	// testEtcdTimeout is used for all RPC calls initiated
	// by the test fixture.
	testEtcdTimeout = 5 * time.Second
)

// EtcdTestFixture holds internal state of the
// etcd test fixture.
...
// NewTestEtcdInstance creates an embedded etcd instance for testing,
// listening on random open ports. Returns the connection config and
// a cleanup func that will stop the etcd instance.
...
// NewTestEtcdTestFixture creates a new etcd-test fixture.
// This is helper object to facilitate etcd tests and ensure
...
// Get queries a key and returns the stored value from the
// test etcd database.
...
// Dump scans and returns all key/values from the test
// etcd database.
...
// BackendConfig returns the backend config for connecting to the
// embedded etcd instance.
func (f *EtcdTestFixture) BackendConfig() BackendConfig {
	return *f.config
}

// Cleanup should be called at test fixture teardown to
// stop the embedded etcd instance and remove all temp db
// files form the filesystem.
