...
// bkey is a helper functon used in tests to create a bucket key
// from passed bucket list.
...
// bval is a helper function used in tests to create a bucket value
// (the value part of a bucket key) from the passed bucket list.
func bval(buckets ...string) string {
	id := makeBucketID([]byte(bkey(buckets...)))
	return string(id[:])
}

// vkey is a helper function used in tests to create a value key
// from the passed key and bucket list.
