...
// RunSTM runs the apply function by creating an STM (using serializable
// serialization), passing it to the apply and handling commit errors
// and retries.
...
// runSTM implements the run loop of the STM, running the
// apply func, catching errors and handling commit. The loop
// will quit on every error except CommitError which is used
// to indicate a necessary retry.
...
// add inserts a txn response to the read set. This is
// useful when the txn fails due to conflict where the
// txn response can be used to prefetch key/values.
...
// fetch is a helper to fetch key/value given options. If
// a value is returned then fetch will try to fix the STM's
// snapshot revision (if not already set). We'll also cache
// the returned key/value in the read set.
...
// If there's no key starting with prefix, Last will
// return nil.
func (s *stm) First(prefix string) (*KV, error) {
	return s.next(prefix, prefix, true)
}

// Last returns the last key/value with prefix. If there's
// no key starting with prefix, Last will return nil.
...
// Prev returns the prior key/value before key (with prefix). If there's
// no such key Next will return nil.
...
// Next returns the next key/value after key (with prefix). If there's
// no such key Next will return nil.
func (s *stm) Next(prefix string, key string) (*KV, error) {
	return s.next(prefix, key, false)
}

// Seek "seeks" to the key (with prefix). If the key doesn't exists
// it'll get the next key with the same prefix. If no key fills this
// criteria, Seek will return nil.
func (s *stm) Seek(prefix, key string) (*KV, error) {
	return s.next(prefix, key, true)
}

// next will try to retrieve the next match that has prefix and starts
// with the passed startKey. If includeStartKey is set to true, it'll
// return the value of startKey (essentially implementing seek).
...
// Put sets the value of the passed key. The actual
// put will happen upon commit.
...
// Del marks a key as deleted. The actual delete will
// happen upon commit.
...
// OnCommit sets the callback that is called upon committing
// the STM transaction.
func (s *stm) OnCommit(cb func()) {
	s.onCommit = cb
}

// commit builds the final transaction and tries to execute
// it. If commit fails because the keys have changed return
// a CommitError, otherwise return a DatabaseError.
...
// Commit simply calls commit and the commit stats callback
// if set.
...
// Rollback resets the STM. This is useful for uncommitted transaction
// rollback and also used in the STM main loop to reset state if commit
// fails.
func (s *stm) Rollback() {
